...
def single_cc_dist(i, j):
    ...
    dist_pair = distance(
        mppool_ini_dict["lat"][i],
        mppool_ini_dict["lon"][i],
        mppool_ini_dict["lat"][j],
        mppool_ini_dict["lon"][j],
    )
...
def station_spatial_correlation(lat, lon, data, num_processes=4):
    ...
    with Pool(
        processes=num_processes, initializer=init_worker, initargs=(data, lat, lon)
    ) as pool:
...
def station_space_time_correlation(config):
    ...
    for vn in range(len(target_vars)):
        ...
        with xr.open_dataset(file_allstn) as ds_stn:
            ...
            if (len(ds_stn.time) < 12 * rolling_window) and (
                auto_corr_method_vn == "anomaly"
            ):
                auto_corr_method_vn = "direct"
                print(
                    f"Station data length is too short -- changing auto_corr_method from anomaly to direct."
                )
            ...
            if auto_corr_method_vn == "direct":
                ...
                stn_value_mv = (
                    ds_stn[var_name]
                    .rolling(time=rolling_window, center=True)
                    .mean()
                    .values
                )
                ...
                print("Unknown auto_corr_method: entry=", auto_corr_method_vn)
                sys.exit()
        ...
        if not clen_config[vn] > 0:
            ...
            cc_pair, dist_pair, index_pair = station_spatial_correlation(
                lat, lon, stn_value, num_processes
            )
            ...
            ds_out[f"{var_name}_space_pair_dist"] = xr.DataArray(
                dist_pair, dims=("pair")
            )
            ds_out[f"{var_name}_space_pair_index"] = xr.DataArray(
                index_pair, dims=("pair", "pind")
            )
            ds_out[f"{var_name}_space_Clen"] = xr.DataArray([popt[0]], dims=("z"))
            # ds_out[f'{var_name}_space_param2'] = xr.DataArray([popt[1]], dims=('z'))

            print(
                f"The mean spatial correlation length (Clen) for {var_name} is {popt[0]}"
            )
    ...
    for var1, var2 in linkvar.items():
        ...
        with xr.open_dataset(file_allstn) as ds_stn:
            ...
            if auto_corr_method_vn == "direct":
                ...
                stn_value_mv = (
                    ds_stn[var1].rolling(time=rolling_window, center=True).mean().values
                )
                data1 = stn_value_raw - stn_value_mv

                stn_value_raw = ds_stn[var2].values
                stn_value_mv = (
                    ds_stn[var2].rolling(time=rolling_window, center=True).mean().values
                )
        ...
        ds_out[f"{var1}_{var2}_cc_cross_mean"] = xr.DataArray(
            [np.nanmean(cc)], dims=("z")
        )
