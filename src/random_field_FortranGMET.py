...
def spcorr_grd(grid_lat, grid_lon, clen, outfile):
    ...
    spcorr_ipos = np.empty(
        (nspl1, nspl2), dtype=object
    ...
    for ires in range(nnst - 1, 0 - 1, -1):
        incr = (
            2**ires
        )  # increment(2 ** 4 = 16, 2 ** 3 = 8, 2 ** 2 = 4, 2 ** 1 = 2, 2 ** 0 = 1)
        ...
        for isp1 in range(0, nspl1, incr):
            for isp2 in range(0, nspl2, incr):
                ...
                if not gmsk[isp1, isp2]:
                    ...
                    for jsp1 in range(
                        max(0, isp1 - (incr * nloc)),
                        min(isp1 + (incr * nloc) + 1, nspl1),
                    ):  # doubt
                        for jsp2 in range(
                            max(0, isp2 - (incr * nloc)),
                            min(isp2 + (incr * nloc) + 1, nspl2),
                        ):  # doubt
                    ...
                    if k >= 2:
                        ...
                        for iprev in range(0, k):
                            for jprev in range(0, iprev + 1):
                                if iprev == jprev:
                                    ...
                                    lon1 = np.deg2rad(
                                        grid_lon[ipos[iprev], jpos[iprev]]
                                    )  # NOTE, iprev, lon
                                    lon2 = np.deg2rad(
                                        grid_lon[ipos[jprev], jpos[jprev]]
                                    )  # NOTE, jprev, lon
                                    lat1 = np.deg2rad(
                                        grid_lat[ipos[iprev], jpos[iprev]]
                                    )  # NOTE, iprev, lat
                                    lat2 = np.deg2rad(
                                        grid_lat[ipos[jprev], jpos[jprev]]
                                    )  # NOTE, jprev, lat
                        ...
                        # all other points
                            ...
                            sdev = np.sqrt(
                                1.0 - np.dot(gvec[0 : k - 1], twgt[0 : k - 1])
                            )
                    gmsk[isp1, isp2] = True
                    # -------------------------------------------------------------------------------------
    ...
    np.savez_compressed(
        outfile,
        spcorr_ipos=spcorr_ipos,
        spcorr_jpos=spcorr_jpos,
        spcorr_wght=spcorr_wght,
        spcorr_sdev=spcorr_sdev,
        iorder=iorder,
        jorder=jorder,
    )
...
def field_rand(
    spcorr_jpos, spcorr_ipos, spcorr_wght, spcorr_sdev, iorder, jorder, seed=np.nan
):
    # ----------------------------------------------------------------------------------------
    ...
    nlon, nlat = (
        spcorr_jpos.shape
    )  # nlon or nlat just represents row/col, not related to real lat/lon
