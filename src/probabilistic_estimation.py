...
def perturb_estimates_withoccurrence(
    data, uncert, poe, rndnum, minrndnum=-3.99, maxrndnum=3.99
):
    data = data.copy()
    uncert = uncert.copy()
    poe = poe.copy()
...
def spcorr_structure(config):
    ...
    file_ens_prefix = (
        f"{path_ensemble}/Ensemble_estimate_"  # this may get overwritten before use
    )
    ...
    for var_name in target_vars:
        ...
        if os.path.isfile(file_spcor):
            ...
            if overwrite_spcorr == True:
                ...
                rf_FGMET.spcorr_grd(
                    grid_lat, grid_lon, allvar_clen[var_name], file_spcor
                )
            else:
...
def generate_prob_estimates_serial(config, member_range=[]):
    ...
    print(
        f"Starting probabilistic estimation of {target_vars} with linked vars {linkvar} for member range {member_range_2}"
    )
    ...
    if master_seed >= 0:
        master_seed = (
            master_seed + date0
        )  # ensure different input batches have different seeds
    else:
        master_seed = np.random.randint(1e9)

    seeds_rf = generate_random_numbers(
        master_seed, len(target_vars) * ensemble_number * ntime
    )
    ...
    with xr.open_dataset(file_stn_cc) as ds_stn_cc:
        for i in range(len(target_vars)):
            ...
            if lag1_auto_cc[i] >= -1:
                ...
                allvar_auto_lag1_cc[var_name] = ds_stn_cc[
                    var_name + "_cc_lag1_mean"
                ].values
        ...
        for vn in range(len(target_vars)):
            ...
            if len(transform_vars[vn]) > 0:
                ...
                if var_name_trans in ds_grid_reg.data_vars:
                    allvar_reg_estimate[var_name] = ds_grid_reg[
                        var_name + "_" + transform_vars[vn]
                    ].values
                elif var_name in ds_grid_reg.data_vars:
                    print(
                        f"Cannot find {var_name_trans} but find {var_name} in {file_grid_reg}"
                    )
                    print(
                    ...
                    if transform_vars[vn] == "ecdf":
                        ...
                        allvar_reg_estimate[var_name] = data_transformation(
                            ds_grid_reg[var_name].values,
                            transform_vars[vn],
    ...
    with xr.open_dataset(file_grid_auxiliary) as ds_grid_aux:
        for vn in range(len(target_vars)):
            var_name = target_vars[vn]
            if len(transform_vars[vn]) > 0:
                allvar_reg_error[var_name] = ds_grid_aux[
                    "uncert_" + var_name + "_" + transform_vars[vn]
                ].values
                if var_name in target_vars_max_constrain:
                    nearby_stn_max[var_name] = ds_grid_aux[
                        "nearmax_" + var_name + "_" + transform_vars[vn]
                    ].values
                else:
        ...
        if not v in linkvar:
            ...
            if linkvar[f"{v}_crosscc"] >= -1:
                ...
                if cross_cc_varname1 in ds_stn_cc:
                    ...
                    sys.exit(
                        f"Cannot find {cross_cc_varname1} or {cross_cc_varname1} in {file_stn_cc}"
                    )
    ...
    for ens in range(member_range[0], member_range[1]):
        ...
        for vn in range(len(target_vars_independent)):
            ...
            if output_randomfield == True:
                ...
                for i in range(ntime):
                    rndi = rf_FGMET.field_rand(
                        spcorr_jpos[var_name],
                        spcorr_ipos[var_name],
                        spcorr_wght[var_name],
                        spcorr_sdev[var_name],
                        iorder[var_name],
                        jorder[var_name],
                    ...
                    if i == 0:
                        ...
                        random_field[:, :, i] = (
                            random_field[:, :, i - 1] * allvar_auto_lag1_cc[var_name]
                            + np.sqrt(1 - allvar_auto_lag1_cc[var_name] ** 2) * rndi
                        )
                ...
                t1 = time.time()
                ens_estimate = prob_estimate_for_one_var(
                    var_name,
                    allvar_reg_estimate[var_name],
                    allvar_reg_error[var_name],
                    nearby_stn_max[var_name],
                    allvar_poe[var_name],
                    random_field,
                    minrndnum,
                    maxrndnum,
                ...
                for i in range(ntime):
                    ...
                    if i == 0:
                        ...
                        rnd_now = (
                            rnd_prev * allvar_auto_lag1_cc[var_name]
                            + np.sqrt(1 - allvar_auto_lag1_cc[var_name] ** 2) * rndi
                        )
                    ...
                    ens_estimate_step = prob_estimate_for_one_var(
                        var_name,
                        d1,
                        d2,
                        d3,
            ...
            for d in range(len(target_vars_dependent)):
                ...
                if linkvar[var_name_dep] == var_name:
                    ...
                    if output_randomfield == True:
                        ...
                        # generate random numbers
                        t1 = time.time()
                        random_field_dep = np.nan * np.zeros(
                            [nrow, ncol, ntime], dtype=np.float32
                        )
                        for i in range(ntime):
                            rndi = rf_FGMET.field_rand(
                                spcorr_jpos[var_name_dep],
                                spcorr_ipos[var_name_dep],
                                spcorr_wght[var_name_dep],
                                spcorr_sdev[var_name_dep],
                                iorder[var_name_dep],
                                jorder[var_name_dep],
                        ...
                        random_field_dep = (
                            random_field * target_vars_dependent_cross_cc[d]
                            + np.sqrt(1 - target_vars_dependent_cross_cc[d] ** 2)
                            * random_field_dep
                        )
                        del random_field
                        random_field_dep[
                            np.isnan(allvar_reg_estimate[var_name_dep])
                        ] = np.nan
                        t2 = time.time()
                        print("time cost of random field generation:", t2 - t1)
                        ...
                        ens_estimate = prob_estimate_for_one_var(
                            var_name_dep,
                            allvar_reg_estimate[var_name_dep],
                            allvar_reg_error[var_name_dep],
                            nearby_stn_max[var_name],
                            allvar_poe[var_name_dep],
                            random_field_dep,
                            minrndnum,
                            maxrndnum,
                            transform_vars[var_name_dep],
                            transform_settings[transform_vars[var_name_dep]],
                            tartime,
                            config,
                        )
                        ds_out[var_name_dep] = xr.DataArray(
                            ens_estimate, dims=("y", "x", "time")
                        )
                        t2 = time.time()
                        print("time cost of probablistic estimation:", t2 - t1)

                        ds_out[var_name_dep + "_rnd"] = xr.DataArray(
                            random_field_dep, dims=("y", "x", "time")
                        )
                    ###############################
                        ...
                        for i in range(ntime):
                            print(
                                f"Generating time step {i}-{ntime} for {var_name_dep}"
                            )
                            ...
                            if i == 0:
                                ...
                                rnd_now = (
                                    rnd_prev * allvar_auto_lag1_cc[var_name_dep]
                                    + np.sqrt(
                                        1 - allvar_auto_lag1_cc[var_name_dep] ** 2
                                    )
                                    * rndi
                                )
                        ...
                        if output_randomfield == True:
                            print(
                                "output_randomfield is not included due to memory consideration"
                            )
        ...
        if "x" in ds_out.dims and "y" in ds_out.dims:
            ...
            if (np.nanmax(grid_lat_diff) < 1e-10) and (
                np.nanmax(grid_lon_diff) < 1e-10
            ):
    ...
    t2 = time.time()
    print(
        f"Complete prob. estimation (ensemble generation) of {target_vars} with linked vars {linkvar} for member range {member_range_2}."
    )
